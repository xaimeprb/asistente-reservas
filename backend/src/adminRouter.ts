...
adminRouter.post('/tenants', requireAuth(['SUPERADMIN']), async (req, res) => {
  try {
...
adminRouter.get('/tenants', requireAuth(['SUPERADMIN']), async (_req, res) => {
...
adminRouter.get(
  ...
  async (req, res) => {
    ...
    if (!tenant) return res.status(404).json({ ok: false, error: 'No existe' });
